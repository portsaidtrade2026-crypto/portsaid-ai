...
test("Register submits a company in Germany with EUR and opens the account", { timeout: 120000 }, async () => {
  ...
  try {
    ...
    await until(() => evaluate(`!![...document.querySelectorAll('[role="option"]')].find(el => el.textContent.includes('EUR'))`), "EUR option")
    await evaluate(`[...document.querySelectorAll('[role="option"]')].find(el => el.textContent.includes('EUR')).setAttribute('data-registration-option', 'currency')`)
