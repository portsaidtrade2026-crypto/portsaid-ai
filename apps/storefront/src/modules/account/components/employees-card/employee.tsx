  ...
  return (
    <Prompt variant="danger">
      <Prompt.Trigger asChild>
        <Button variant="transparent">{t("Remove")}</Button>
      </Prompt.Trigger>
      <Prompt.Content>
        <Prompt.Header>
          <Prompt.Title>{t("Remove Employee")}</Prompt.Title>
          <Prompt.Description>
            {t("Are you sure you want to remove")}{" "}
            <strong>{employee.customer.email}</strong> {t("from your team? They will no longer be able to purchase on behalf of your company.")}
          </Prompt.Description>
        </Prompt.Header>
        <Prompt.Footer>
...
const Employee = ({
  ...
  const handleSubmit = async () => {
    ...
    await updateEmployee(updateData as StoreUpdateEmployee).catch(() => {
      toast.error(t("Error updating employee"))
    })
    ...
    <div className="flex flex-col">
        ...
        <div className="flex flex-col">
          <Text className=" text-neutral-950 font-medium">
            ...
            {employee.is_admin && (
              <>
                {" • "}
               <span className="text-blue-500">{t("Admin")}</span>
              </>
            )}
          </Text>
      ...
      >
        <div className="flex flex-col gap-y-2">
          <Text className=" text-neutral-950 font-medium">{t("Spending Limit")}</Text>
          <CurrencyInput
        ...
        <div className="flex flex-col gap-y-2">
          <Text className=" text-neutral-950 font-medium">{t("Permissions")}</Text>
          <NativeSelect
            className="bg-white"
            name="permissions"
